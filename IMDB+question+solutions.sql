...
SELECT languages,
       Count(total_votes) AS total_no_votes
FROM   movie m
       INNER JOIN ratings r
               ON m.id = r.movie_id
...
SELECT name AS actress_name, SUM(total_votes) AS total_votes, COUNT(m.id) AS movie_count, 
	...
	RANK() OVER(ORDER BY SUM(avg_rating*total_votes)/SUM(total_votes) DESC) AS actoress_rank
FROM movie AS m 
	INNER JOIN ratings AS r ON m.id=r.movie_id 
	INNER JOIN role_mapping AS rm ON m.id=rm.movie_id 
	INNER JOIN names AS n ON rm.name_id=n.id
...
GROUP BY name
HAVING COUNT(m.id)>=3 ) select * from top_5_actress where actoress_rank <=5;
...
SELECT genre,
       Round(Avg(duration), 2)                      AS avg_duraton,
...
(
           ...
           ON         g.movie_id = r.movie_id
           WHERE      avg_rating > 8
           GROUP BY   genre limit 3), top_5_movies AS
(
           SELECT     genre,
                      year,
                      title                                                                                                                         AS movie_name,
                      Cast(Replace(Replace(Ifnull(worlwide_gross_income, 0), 'INR', ''), '$','') AS DECIMAL(10))                                    AS worlwide_gross_income,
                      Dense_rank() OVER( ORDER BY Cast(Replace(Replace(Ifnull(worlwide_gross_income, 0), 'INR', ''), '$', '') AS DECIMAL(10)) DESC) AS movie_rank
           FROM       movie                                                                                                                         AS m
...
(
           SELECT     d.name_id,
                      ...
                      total_votes,
                      m.date_published,
                      Lead(date_published,1) OVER(partition BY d.name_id ORDER BY date_published,movie_id ) AS next_dates
           ...
           ON         m.id = d.movie_id
           INNER JOIN ratings AS r
           ON         r.movie_id = m.id )SELECT   name_id                       AS director_id,
         NAME                          AS director_name,
         Count(movie_id)               AS number_of_movies,
         Round(Avg(Datediff(next_dates, date_published)),2) AS avg_inter_movie_days,
         Round(Avg(avg_rating),2)               AS avg_rating,
         Sum(total_votes)              AS total_votes,
         Min(avg_rating)               AS min_rating,
         Max(avg_rating)               AS max_rating,
         Sum(duration)                 AS total_duration
FROM     next_published_dates
